...
pub fn stroke_rounded_rectangle<Output: GeometryBuilder<StrokeVertex>>(
    ...
    let nums = radii.iter().map(|&radius| {
        if radius > 0.0 {
    ...
    let round_rec_iter = sides.iter()
        .enumerate()
        .flat_map(|(i, side)| {
            side.iter()
                .chain(round_border_iter(centers[i], angles[i], radii[i], nums.next().unwrap()))
    }); // fails to compile: Point vs &Point. need to figure out how to use trait object here.

    stroke_polyline(true, round_rec_iter, output);
